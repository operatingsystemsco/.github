...
<p>10 is a nice build but you have to merge repositories the 'runtime' is like a ddk keep the src directory. and but the 7 samples oin the src directory.  it's a buffer lab I'm waiting on a invitation or 7 source after the war. Merge repositories with the below gist and make a buffer lab out of Windows Core or the .NET Platform on Github on 10 including the terminal and powershell.
